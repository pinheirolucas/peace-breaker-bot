...
`peace-breaker-bot.iss` is an Inno Setup script used to build a Windows installer that bundles just the built binary — no external runtime dependency to bundle anymore, now that `pkg/opusaudio` decodes and resamples in pure Go (see "Architecture" above). Since the whole module has been cgo-free for a while, `.github/workflows/ci.yaml`'s `build` job cross-compiles `peace-breaker-bot.exe` (`GOOS=windows GOARCH=amd64 CGO_ENABLED=0`) as one more step on the same `ubuntu-latest` runner, rather than needing a dedicated `windows-latest` job just to prove the build works — `release.yaml`'s `binaries` job does the same cross-compile for the actual release artifact. `release-windows.yaml` still runs on `windows-latest`, but that's unrelated to cgo: it's there to invoke Inno Setup's `ISCC.exe`, a native Windows-only tool, to produce the installer.
