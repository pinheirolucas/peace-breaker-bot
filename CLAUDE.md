...
`peace-breaker-bot.iss` is an Inno Setup script used to build a Windows installer that bundles just the built binary, with no external runtime dependency, since `pkg/opusaudio` decodes and resamples in pure Go (see "Architecture" above). `.github/workflows/ci.yaml`'s `build` job builds, tests, and vets on `ubuntu-latest` only; Windows compilation is verified at release time, where `release.yaml`'s `binaries` job cross-compiles `windows/amd64` (`CGO_ENABLED=0`) as part of every tagged release and `release-windows.yaml` runs on `windows-latest` to invoke Inno Setup's `ISCC.exe` and produce the installer. The module has no platform-specific code (no `//go:build` tags, no `_windows.go` files), so the `ubuntu-latest` build already type-checks the same source a Windows build would; those release-time builds are what actually catch a dependency introducing something Windows-incompatible.
