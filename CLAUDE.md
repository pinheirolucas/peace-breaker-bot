...
`peace-breaker-bot.iss` is an Inno Setup script used to build a Windows installer that bundles just the built binary, with no external runtime dependency, since `pkg/opusaudio` decodes and resamples in pure Go (see "Architecture" above). `.github/workflows/ci.yaml`'s `build` job doesn't build for Windows on every push/PR: the module has no platform-specific code (no `//go:build` tags, no `_windows.go` files), so a Windows build breaking while the Linux one succeeds would have to come from a dependency. That's caught at release time instead: `release.yaml`'s `binaries` job cross-compiles `windows/amd64` (`CGO_ENABLED=0`) as part of every tagged release, and `release-windows.yaml` runs on `windows-latest` to invoke Inno Setup's `ISCC.exe` and produce the installer.
